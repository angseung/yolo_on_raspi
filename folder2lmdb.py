...
class ImageFolderLMDB(data.Dataset):
    ...
    def get_single_image(self, index, expand=False, expand_scale=1.5):
        ...
        old_dims = torch.FloatTensor(
            [new_img.width, new_img.height, new_img.width, new_img.height]
        ).unsqueeze(0)
    ...
    def show_image(
        ...
        if gray_img_only == True:
            ...
            if resize != None:
                cv_img = cv2.resize(
                    cv_img, (resize[0], resize[1]), interpolation=cv2.INTER_AREA
                )
    ...
    def collate_fn(self, batch):
        ...
        for b in batch:
            if self.has_seg:
                maps = torch.zeros(
                    seg_random_size[0], seg_random_size[1], self.seg_num_classes
                )
                for i, m in enumerate(b[3]):
                    cv_img = np.array(m.convert("L"))
                    cv_img = cv2.resize(
                        cv_img,
                        (seg_random_size[0], seg_random_size[1]),
...
def folder2lmdb(dataset_path, write_frequency=5000):
    ...
    if segmentation_enable:
        trainval_dataset = DatasetFromFile(
            trainval_dataset_path["imgs"],
            trainval_dataset_path["annos"],
            trainval_dataset_path["segs"],
            trainval_dataset_path["lists"],
            classes_name,
            dataset_name=trainval_dataset_path["name"],
            phase="test",
            ...
            ext_anno=ext_anno,
            ext_seg=ext_seg,
            ori_classes_name=ori_classes_name,
        )

        test_dataset = DatasetFromFile(
            test_dataset_path["imgs"],
            test_dataset_path["annos"],
            test_dataset_path["segs"],
            test_dataset_path["lists"],
            classes_name,
    ...
    for i in range(len(total_set)):
        ...
        for idx, data in enumerate(data_loader):
            if segmentation_enable:
                ...
                txn.put(
                    "{}".format(idx).encode("ascii"), pickle.dumps((image, label, seg))
                )
            else:
                image, label = data[0][0], data[0][1]
                txn.put("{}".format(idx).encode("ascii"), pickle.dumps((image, label)))
            sum += len(label)

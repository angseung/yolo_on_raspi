...
class YOLOLoss(nn.Module):
    ...
    def pre_maps(self, bs, is_cuda, anchors, in_w, in_h):
        ...
        anchor_w = (
            anchor_w.repeat(bs, 1)
            .repeat(1, 1, in_h * in_w)
            .view(bs, self.num_mask, in_h, in_w, 1)
            .to(device)
        )
        anchor_h = (
            anchor_h.repeat(bs, 1)
            .repeat(1, 1, in_h * in_w)
            .view(bs, self.num_mask, in_h, in_w, 1)
            .to(device)
        )
    ...
    def get_target(
        self, target, input, anchors, in_w, in_h, ignore_threshold, iou_thresh=0.5
    ):

        bs = input.size(0)
        this_anchors = np.array(anchors)[self.mask]
        FloatTensor = torch.cuda.FloatTensor if input.is_cuda else torch.FloatTensor
        targets_weight = torch.zeros(
            bs, self.num_mask, in_h, in_w, self.num_classes + 1, requires_grad=False
        ).to(device)
        pred_boxes = torch.zeros(
            bs, self.num_mask, in_h, in_w, 0, requires_grad=False
        ).to(device)
        prediction = (
            input.view(bs, self.num_mask, self.bbox_attrs, in_h, in_w)
            .permute(0, 1, 3, 4, 2)
            .contiguous()
        )
        ...
        pred_boxes = torch.cat(
            (pred_boxes, (xy + grid_xy) / FloatTensor([in_w, in_h])), 4
        )
        ...
        for b in range(bs):
            ...
            pred_boxes2 = (
                pred_boxes[b].view((in_w * in_h * self.num_mask, 4)).to(device)
            )
            ...
            for t in range(len(target[b])):
                ...
                for k in range(self.num_mask):
                    if k == bn or iou_thresh_list[k] == True:
                        ...
                        cls_score = (
                            cls_score + output[b, k, gj, gi, 1 + cls_index].item()
                        )
        if count > 0:
            obj_avg = obj / count
    ...
    def get_pred_boxes(self, input, anchors, in_w, in_h):
        ...
        pred_boxes = torch.cat(
            (pred_boxes, score.unsqueeze(4), cls_idx.float().unsqueeze(4)), 4
        )
    ...
    def forward(self, input, targets=None):
        ...
        # print(input.shape)
        scaled_anchors = [
            (a_w / self.img_size[0], a_h / self.img_size[1])
        ...
        if targets is not None:
            ...
            iou_loss = torch.Tensor([0]).to(device)
            if iou_losses.size(0) > 0:
                iou_loss = (
                    self.weighted_mse_loss(iou_losses, iou_target, iou_weights)
                    / iou_losses.numel()
                )
    ...
    def box_ciou(self, box1, box2):
        ...
        w1, h1 = (box1[..., 2] - box1[..., 0]).unsqueeze(1), (
            box1[..., 3] - box1[..., 1]
        ).unsqueeze(1)
        w2, h2 = (box2[..., 2] - box2[..., 0]).unsqueeze(1), (
            box2[..., 3] - box2[..., 1]
        ).unsqueeze(1)
        x1, y1 = (box1[..., 2] + box1[..., 0]).unsqueeze(1) / 2, (
            box1[..., 1] + box1[..., 3]
        ).unsqueeze(1) / 2
        x2, y2 = (box2[..., 2] + box2[..., 0]).unsqueeze(1) / 2, (
            box2[..., 1] + box2[..., 3]
        ).unsqueeze(1) / 2

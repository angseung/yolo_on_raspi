...
class yolo(nn.Module):
    def __init__(self, config):
        ...
        self.yolo_headS32 = yolo_head(
            [960, self.num_anchors * (5 + self.num_classes)], 320
        )
        ...
        self.yolo_headS16 = yolo_head(
            [640, self.num_anchors * (5 + self.num_classes)], 320
        )
        ...
        for i in range(2):
            self.yolo_losses.append(
                YOLOLoss(
                    ...
                    config["yolo"]["ignore_thresh"][i],
                    config["yolo"]["iou_thresh"],
                    iou_weighting=config["iou_weighting"],
                )
            )

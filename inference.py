...
parser.add_argument(
    "-c",
    "--checkpoint",
    default="checkpoint/checkpoint.pth.tar",
    type=str,
    metavar="PATH",
    help="path to load checkpoint (default: checkpoint/checkpoint.pth.tar)",
)
parser.add_argument(
    "-e", "--export", default="", type=str, metavar="PATH", help="path to export model"
)
parser.add_argument(
    "-y",
    "--data_yaml",
...
def main(args):
    ...
    if kind.extension in ["png", "jpg", "jpeg", "tiff", "bmp", "gif"]:
        ...
        for cls in range(seg_map.shape[0]):
            seg_maps.append(
                cv2.resize(
                    seg_map[cls, ...], (width, height), interpolation=cv2.INTER_LINEAR
                )
            )
        ...
        for idx, map in enumerate(seg_maps):
            mask = map > 0.5
            annotated_image[..., color_channel[idx]][mask] = annotated_image[
                ..., color_channel[idx]
            ][mask] * (1.0 - map[mask])

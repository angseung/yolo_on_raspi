...
def main(args):
    ...
    optimizer = optim.AdamW(
        params=params, lr=args.learning_rate, weight_decay=args.weight_decay
    )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch in args.schedule:
            ...
            save_checkpoint(
                ...
                filename="epoch%d_checkpoint.pth.tar" % epoch,
                export_path=args.export,
            )
            adjust_learning_rate(optimizer, 0.5)
            print(
                "adjusted to current lr: "
                "{}".format(
        ...
        train_loss, iou = train(
            train_loader, model, optimizer, epoch, sampler, segmentation_enable
        )
...
def train(train_loader, model, optimizer, epoch, sampler, segmentation_enable):
    ...
    for batch_idx, (images, targets, total_num, seg_maps) in enumerate(train_loader):
        ...
        # end = time.time()
        if segmentation_enable:
            bar.suffix = "%(percent)3d%% | {total:} | {loss:.4f} | {cnt:2.1f} | {iou:.3f} | {obj:.3f} | {no_obj:.4f} | {cls:.3f} | {rec:.4f} | {seg_obj:.3f} | {seg_no_obj:.6f} |".format(
                total=bar.elapsed_td,
                loss=losses.avg,
                cnt=(count[0].avg + count[1].avg),
                iou=(iou[0].avg + iou[1].avg) / 2.0,
                obj=(obj[0].avg + obj[1].avg) / 2.0,
                no_obj=(no_obj[0].avg + no_obj[1].avg) / 2.0,
                cls=(cls_score[0].avg + cls_score[1].avg) / 2.0,
                rec=(recall[0].avg + recall[1].avg) / 2.0,
                seg_obj=seg_obj.avg,
                seg_no_obj=seg_no_obj.avg,
            )
        else:
            bar.suffix = "%(percent)3d%% | {total:} | {loss:.4f} | {cnt1:2.1f} | {iou1:.3f} | {obj1:.3f} | {no_obj1:.4f} | {cls1:.3f} | {rec1:.3f}  | {cnt2:2.1f}  | {iou2:.3f} | {obj2:.3f} | {no_obj2:.4f}  | {cls2:.3f}  | {rec2:.3f}   |".format(
                # batch=batch_idx + 1,
...
def test(
    test_loader, model, optimizer, epoch, config, classes_name, segmentation_enable
):
...
def get_params():
    ...
    parser.add_argument(
        ...
        dest="evaluate",
        action="store_true",
        help="Evaluate mAP? default=False",
    )
    parser.add_argument(
        "--mosaic_num",
        default=None,
        type=int,
        help="mosaic number in image augmentation",
    )
    parser.add_argument(
        "--ignore_thresh_1", default=None, type=float, help="ignore layer 1"
    )
    parser.add_argument(
        "--ignore_thresh_2", default=None, type=float, help="ignore layer 2"
    )
    parser.add_argument(
        "--iou_thresh", default=None, type=float, help="ignore iou thresh"
    )
    parser.add_argument(
        "--expand_scale",
        default=None,
        type=float,
        help="image augmentation expand scale",
    )
    parser.add_argument(
        "--iou_weighting", default=None, type=float, help="iou loss weighting"
    )
    args = parser.parse_args()
    return args
